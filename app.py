...
@app.route("/new_word_discovery", methods=['GET'])
def new_word_home():
    return render_template("new_word_discovery.html")


#新词挖掘-获取前端输入数据        
@app.route('/new_word_discovery', methods=['POST'])
def get_newword_input_text():
    usertext = request.form.get('inputtext')
    wordcount = request.form.get('wordcount')
    wordlength = request.form.get('wordlength')
    
    try:
        if usertext != None and wordcount !=None and wordlength!= None:
            content = usertext + '\n, ' + wordcount + '\n, ' + wordlength
            save_to_file(config.new_word_discovery_file_save_path, content)
            print(content)
        return redirect('/download_newword')
    except:
        return render_template("new_word_discovery.html")        
    

#新词挖掘-数据请求
@app.route('/download_newword', methods=['GET'])  
def rt_newword_input_text():
    cur = exe_03.rt_new_word_discovery_base()
    return  render_template("gennewword.html", curinput=cur)      
    
#新词挖掘-下载输出结果
@app.route('/download_newword', methods=['POST'])
def download_newword():
...
## Part 5 文本生成
@app.route("/generation_1")
def text_summary_home(): 
    return render_template("text_generation_1.html")
